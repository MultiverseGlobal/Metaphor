...
class ReflectionService:
    ...
    async def generate_ambiguities(self, data_samples: List[str]) -> List[str]:
        """Generates 3 dynamic clarifying questions based on raw ingested data."""
        if not data_samples:
            return [
                "What is the primary mission of this workspace?",
                "Which project requires the most immediate focus?",
                "Are there any specific constraints I should know about?"
            ]
            
        combined_text = "\n\n".join(data_samples)[:8000] # Cap length
        system_prompt = (
            "You are the Metaphor Context Engine. The user is connecting their data sources for the first time.\n"
            "Your job is to read these raw data samples (e.g., commits, docs) and generate exactly 3 clarifying questions.\n"
            "These questions should ask the user to clarify ambiguities, overlap between projects, or confirm core goals based on the data.\n"
            "Output a JSON array of 3 strings."
        )
        prompt = f"Raw Data:\n{combined_text}\n\nOutput JSON array of 3 questions:"
        
        try:
            response_text = await llm_service.query_llm(prompt=prompt, system_prompt=system_prompt, temperature=0.7)
            clean = response_text.strip()
            if clean.startswith("```json"): clean = clean[7:]
            if clean.startswith("```"): clean = clean[3:]
            if clean.endswith("```"): clean = clean[:-3]
            ...
            return [
                "I saw some activity in your data sources. Which project is the highest priority right now?",
                "Are there overlapping domains between these data sources?",
                "What's the main goal you are trying to achieve this week?"
            ]
        except Exception as e:
            logger.error(f"Failed to generate ambiguities: {e}")
            return [
                "What is the primary mission of this workspace?",
                "Which project requires the most immediate focus?",
                "Are there any specific constraints I should know about?"
