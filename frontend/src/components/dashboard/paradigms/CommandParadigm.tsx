...
export default function CommandParadigm() {
    ...
    <div className="w-full h-full relative overflow-hidden">
      ...
      <div className={`absolute z-50 transition-all duration-700 ease-in-out w-full max-w-2xl px-6 ${
        ...
        <form onSubmit={handleCommand} className="relative group">
          <div className="absolute -inset-1 bg-gradient-to-r from-primary-dim to-transparent rounded-2xl blur-xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition duration-500"></div>
          <div className="relative bg-surface-1/50 backdrop-blur-3xl border border-border-strong rounded-2xl shadow-glass flex items-center p-2">
            ...
            <input 
              ...
              placeholder="Query the Matrix (e.g., 'monitor health')" 
              className="flex-1 bg-transparent border-none text-lg text-foreground placeholder:text-muted/50 py-3 outline-none"
            />
            <div className="px-3 py-1.5 mr-2 bg-surface-2 rounded border border-border-subtle text-xs font-mono text-muted hidden sm:block">
              ↵ Enter
            </div>
          </div>
        </form>
      ...
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary-dim rounded-full blur-[150px] transition-opacity duration-1000 pointer-events-none ${activeLayout === "void" ? "opacity-100" : "opacity-0"}`}></div>
    </div>
  );
}
